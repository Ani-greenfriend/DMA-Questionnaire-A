...
| ratings | One row per (IRO × criterion) per submission; value empty means skipped | id, submission_id, assessment_id, iro_id, criterion_key, value, justification |
...
| assessments | Unauthenticated (anon) | Only the assessment matching a personal link (so Tool A can render) | No | No | No |
| topic_library | Authenticated user | All rows | Yes | Yes | Yes |
| iros | Authenticated user | All rows | Yes | Yes | Yes |
| iros | Unauthenticated (anon) | Only rows of the assessment matching a personal link | No | No | No |
| stakeholder_groups | Authenticated user | All rows | Yes | Yes | Yes |
| stakeholder_groups | Unauthenticated (anon) | Only the names and perspectives offered for the matching assessment | No | No | No |
...
| invitations | Unauthenticated (anon) | No direct read — narrow lookup by link code only (Tool A) | No | Status and timestamps only, by link code | No |
...
| submissions | Unauthenticated (anon) | Only the matching draft or submitted row (Tool A) | Yes — one per invitation | Only while a draft | No |
| ratings, topic_justifications | Authenticated user | All rows | Yes (live sessions) | Only while the submission is a draft | No |
| ratings, topic_justifications | Unauthenticated (anon) | Only the matching draft's rows (Tool A) | Yes | Only while the submission is a draft | No |
